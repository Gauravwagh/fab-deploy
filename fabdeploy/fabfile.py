...
# Config setup #
################
USER_MODEL = 'auth.User' # provide user user model here its similar what we put in settings.py file
conf = {}
...
env.proj_app = "fabdeploy"

##################
# Client app defaults
##################
env.app_name = conf.get("APP_NAME", "fabdeploy")
env.app_repo_url = conf.get("APP_REPO_URL", "")
env.app_dir = "project-front"
env.dist_path = "%s/%s/%s/%s/%s" % (env.venv_home, env.app_name, env.app_dir, "source", "dist")
...
templates = {
    ...
    "settings": {
        ...
        "remote_path": "%(proj_path)s/fabdeploy/local_settings.py",
    },
}
...
def deploy():
    ...
    with project():
        ...
        with update_changed_requirements():
            ...
            # pull dev branch
            #run("git pull origin dev_v1.2 -f" if git else "hg pull && hg up -C")
...
def deploy_app():
    ...
    with app_project():
        git = env.git
        run("git pull origin develop -f" if git else "hg pull && hg up -C")
        with cd("source"):
           run("npm install")
